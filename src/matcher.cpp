...
bool Matcher::findMatchSeed(const Seed &seed, const Frame &cur_frame, Vector2d &px_cur, float ncc_thresh) {
  ...
  Vector3d ref_dir(seed.ftr->frame->pos() - seed_pos);
  ref_dir.normalize();
  Vector3d cur_dir(cur_frame.pos() - seed_pos);
  cur_dir.normalize();
...
bool Matcher::findEpipolarMatchDirect(
  ...
  for (size_t i = 0; i < n_steps; ++i, uv += step) {
    ...
    pxi = Vector2i(px[0] / (1 << search_level_) + 0.5,
...
int Matcher::doLineStereo(
  ...
  if ((ref_ftr.type == Feature::GRADIENT || ref_ftr.type == Feature::EDGELET)
...
bool Matcher::KLTLimited2D(
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    int u_r = floor(bestU);
    int v_r = floor(bestV);
    if (u_r < halfPatchSize || v_r < halfPatchSize || u_r >= targetImg.cols - halfPatchSize
        || v_r >= targetImg.rows - halfPatchSize)
      break;

    if (isnan(bestU)
        || isnan(bestV)) // TODO very rarely this can happen, maybe H is singular? should not be at corner.. check
      return false;
