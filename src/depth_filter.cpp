...
void DepthFilter::addKeyframe(FramePtr frame, double depth_mean, double depth_min, float converge_thresh) {
  ...
  if (thread_ != nullptr) {
    ...
    initializeSeeds(frame);  // 一般开启了深度滤波线程的时候，不会执行这里的else部分
  }
}
...
void DepthFilter::initializeSeeds(FramePtr frame) {
  ...
  // TODO：batch_counter计数，相当于是关键帧的ID，只不过是从 1 开始记录id --by wzh
  ++Seed::batch_counter;
  ...
  std::for_each(new_features.begin(), new_features.end(), [&](Feature *ftr) {
    ...
    // HSO论文里面的图12：取当前关键帧和上一关键帧之间的普通帧（这些普通帧相对于当前关键帧来说比较老）。
    // 目的-> 加速地图点的收敛
    for (auto it = frame_prior_[Seed::batch_counter - 1].begin(); it != frame_prior_[Seed::batch_counter - 1].end();
         ++it) {
...
void DepthFilter::updateSeedsLoop() {
  while (!boost::this_thread::interruption_requested()) //当有请求interrupt时终止
  {
    FramePtr frame;
    {
      // 检测是否有线程锁frame_queue_
      lock_t lock(frame_queue_mut_);

      // 如果种子链表为空 且 没有普通帧和关键帧加入，就上锁，等待通知唤醒
      // 如果种子链表不为空 且 普通帧队列为空 且 没有插入新关键帧，就遍历种子链表，
      if (seeds_.empty()) {
        // 没有普通帧和关键帧加入, 就上锁, 等待 notify
      ...
      // 如果普通帧的队列不为空 或者 有新的关键帧插入
      // 如果普通帧的队列为空 且 没有插入新的关键帧 那么就等待通知唤醒
      if (!frame_queue_.empty() || new_keyframe_set_) {
        // 如果有新的关键帧插入，则清空普通帧队列，优先处理处理关键帧。
        // 如果没有新的关键帧插入，则从普通帧队列中取出队头帧（最先插入队列的帧）
        if (new_keyframe_set_) {
          new_keyframe_set_ = false; // 处理了新的关键帧就置false
          seeds_updating_halt_ = false; // 不能停止种子更新(halt->停止)
          clearFrameQueue(); // 清空普通帧队列
          frame = new_keyframe_;
        } else {
      ...
      // 这两个变量是中间变量，用来看observeDepthWithPreviousFrameOnce()函数执行过程的
      n_pre_try_ = 0; // 用来统计加速种子收敛的之前的普通帧的数量
      n_pre_update_ = 0; // 极线匹配成功的普通帧数量
...
void DepthFilter::updateSeeds(FramePtr frame) {
  // 如果该帧不是关键帧（那么就是普通帧），那么就将该普通帧插入到上一关键帧后的普通帧链表 头部
  // 如果该帧是关键帧，那么就将新的关键帧加入到该关键帧下的普通帧链表里。why？
  if (!frame->isKeyframe())
    frame_prior_[Seed::batch_counter].push_front(frame);
  else
    frame_prior_[Seed::batch_counter + 1].push_front(frame);

  // 如果队列的关键帧数量大于5 且 该帧是关键帧
  // 操作->将前5关键帧后的普通帧链表里的普通帧删掉，保留关键帧？
...
void DepthFilter::observeDepth() {
  threadReducer_->reduce(boost::bind(&DepthFilter::observeDepthRow, this, _1, _2, _3),
                         0,
                         (int) seeds_.size(),
                         runningStats_,
                         10);
...
void DepthFilter::observeDepthWithPreviousFrameOnce(std::list<Seed>::iterator &ite) {
  ...
  if (!matcher_.findEpipolarMatchPrevious(*ite->ftr->frame,
                                          *preFrame,
                                          *ite->ftr,
                                          1.0 / ite->mu,
                                          1.0 / z_inv_min,
...
bool DepthFilter::activatePoint(Seed &seed, bool &isValid) {
  ...
  vector<Vector2d> targetNormal;
  targetNormal.reserve(targets.size());
