...
void FastDetector::detect(
    ...
    Features &fts) {
  Corners corners(grid_n_cols_ *grid_n_rows_, Corner(
  0, 0, detection_threshold, 0, 0.0f));
  for (int L = 0; L < n_pyr_levels_; ++L) {
    const int scale = (1 << L);
    vector<fast::fast_xy> fast_corners;
    fast::fast_corner_detect_plain_8(
        (fast::fast_byte *) img_pyr[L].data,
        img_pyr[L].cols,
        img_pyr[L].rows,
        img_pyr[L].cols,
        detection_threshold,
...
// #endif
    ...
    fast::fast_corner_score_8((fast::fast_byte *) img_pyr[L].data,
                              img_pyr[L].cols,
                              fast_corners,
                              detection_threshold,
                              scores);
...
void EdgeletDetector::detect(
    ...
    const float &detection_threshold,
    Features &fts) {
  EdgeLets edgelets(grid_n_cols_ *grid_n_rows_, EdgeLet(Vector2i(
  0, 0), Vector2i(0, 0), 0, 0.0f, false));
  Vector2d normal;
...
void GradientDetector::detect(
    ...
    const float &detection_threshold,
    Features &fts) {
  Gradients gradients(grid_n_cols_ *grid_n_rows_, Gradient(
  0, 0, 0, 0));
  // for(int L = 0; L < n_pyr_levels_; ++L)
  // {
  //   const int scale = (1<<L);
...
FeatureExtractor::FeatureExtractor(
  ...
  if (isInit)
    ...
    //Config::maxFts()为200，应该就是论文里 5.7 Candidate point extraction 中特征点提取时用到的阈值 N_th=200
...
void FeatureExtractor::detect(
  ...
  // 初始化的时候nFeatures_ = 2000,正常跟踪的时候nFeatures_=300
  // extFeatures_：提取
  ...
  // 经过四叉树操作，可以使特征点的分布更加均匀，从而减少特征点的冗余度
  ...
  for (size_t i = 0; i < resultFeatures_.size(); ++i) {
    ...
    if (keyPoint.species == kCornerHigh)
      ...
      // 对于Feature::GRADIENT来说，x和y方向的梯度为0
...
// fast角点提取函数，并计算了响应值（shi-tomasi分数）
void FeatureExtractor::fastDetect(const ImgPyr &img_pyr) {
  ...
  for (int L = 0; L < nLevels_; ++L) {
    ...
    //每一层金字塔相对于最底层（原始层）金字塔缩放的比例因子
    const int scale = (1 << L);
    vector<fast::fast_xy> fastCorners;

    // 对图像金字塔进行角点检测（检测连续9个角点）
    fast::fast_corner_detect_9_sse2(
        (fast::fast_byte *) img_pyr[L].data,
        img_pyr[L].cols,
        img_pyr[L].rows,
    ...
    // 输入检测出的角点特征，以及得分值，执行非极大值抑制操作
    //非极大值抑制：若以某特征点为中心的一个领域内（这里是3x3），该特征点的响应值最高（得分最高），则保留，否则抑制。
