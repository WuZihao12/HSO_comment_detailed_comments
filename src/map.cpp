...
void Map::safeDeleteTempPoint(pair<Point *, Feature *> &p) {
  if (p.first->seedStates_ == -1) {
    if (p.first->isBad_)
      ...
      // update pos
      p.first->pos_ = p.first->hostFeature_->frame->T_f_w_.inverse()
          * (p.first->hostFeature_->f * (1.0 / p.first->idist_));
      ...
      if (p.first->obs_.size() == 1) {
        ...
        // thread safe
        point_candidates_.candidates_.push_back(MapPointCandidates::PointCandidate(p.first,
                                                                                   p.first->obs_.front()));
      } else {
        // update to unknow
  ...
  for (const auto &kf : keyframes_) {
    ...
    for (const auto &keypoint : kf->key_pts_) {
      ...
      if (frame->isVisible(keypoint->point->pos_)) {
        ...
        // 插入共视关键帧 以及 当前帧和关键帧之间的距离（当前帧和共视关键帧都在世界坐标系下（看下标最后面）,因此可以直接计算平移距离）
        close_kfs.push_back(std::make_pair(kf, (frame->T_f_w_.translation() - kf->T_f_w_.translation()).norm()));
        // 找到共视点后就插入共视关键帧，跳出循环，继续判断下一个关键帧是否和当前帧共视。
