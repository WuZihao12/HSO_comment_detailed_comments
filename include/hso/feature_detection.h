...
// 特征检测命名空间下的特征提取类
// feature_detection::FeatureExtractor
...
/*  void gradDetect(const ImgPyr &img_pyr);
  ...
  int cellSize_; // 一个网格大小
  int nLevels_;  // 图像金字塔层数

  int nFeatures_; //提取的总特征数量
  int extFeatures_;  //当前已经提取到的特征数数量
  int needFeatures_;  //需要提取的特征数量
  ...
  static const int gridSize_ = 8; // 第0层图像金字塔划分的方格边长，接下来的每层将按照上一层边长的一半进行划分
  std::vector<int> vGrids_; // 每层图像金字塔按vGrids_*vGrids_大小的方格来进行划分
  std::vector<int> vGridCols_; // 图像金字塔的高可以分成多少份
  std::vector<int> vGridRows_; // 图像金字塔的宽可以分成多少份
  std::vector<vector < bool> >haveFeatures_; // 用于存放每一层图像金字塔的方格，即将每一层的方格排成一个列向量/行向量
