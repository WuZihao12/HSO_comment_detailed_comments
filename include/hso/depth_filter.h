...
struct Seed
{
    ...
    static int batch_counter;//每初始化一次(增加关键帧)就加一个batch，相当于关键帧的ID
    static int seed_counter;//总的种子点数量
    int batch_id; //每当种子被创建的时候，关键帧的batch id             //!< Batch id is the id of the keyframe for which the seed was created.
    int id; //种子id,仅用于观测                    //!< Seed ID, only used for visualization.
    Feature* ftr; //关键帧中深度应该被计算的特征                //!< Feature in the keyframe for which the depth should be computed.
    float a; //Beta分布，越高，内点的概率越大                    //!< a of Beta distribution: When high, probability of inlier is large.
    float b; //Beta分布，越高，外点的概率越大                    //!< b of Beta distribution: When high, probability of outlier is large.
    float mu; //正态分布的均值                   //!< Mean of normal distribution.
    float z_range; //可能深度的最大范围              //!< Max range of the possible depth.
    float sigma2; //正态分布的方差               //!< Variance of normal distribution.
    Matrix2d patch_cov; //参考图像中，patch的协方差          //!< Patch covariance in reference image.
...
// 可以参考这篇csdn的注释https://blog.csdn.net/qq_43303411/article/details/113790803
class DepthFilter
{
